...
const int DCACHE_SIZE  = 16*1024*1024;/*Harry*/
...
int memcpy_test(int argc, char** argv)
{
...
#if 1 /*Harry: I want to add samples*/
    //const int MAX_SIZE = 4096*1024; // 4MB
    ...
    if (option == 0) {
        ...
        for (int i=0 ; ; i++) {
            if (!fast) {
                if (size<128)          size += 8;
#if 0 /*Harry: I want to add samples for assuming cacheline=64B*/
                else if (size<32768)   size += 64;  /*assume L1=32KB, plot cacheline size*/
                else if (size<524288)  size += 128; /*assume L2=512KB, plot AXI width*/
                else                   size += 256;
#else
